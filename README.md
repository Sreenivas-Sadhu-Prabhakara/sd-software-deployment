...
| **Asset Type** | Software Release |
| **Control Record** | Software Release Procedure |
...
| POST | `/v1/software-release-procedure/initiate` | Initiate |
| GET | `/v1/software-release-procedure` | Retrieve (list) |
| GET | `/v1/software-release-procedure/{crId}/retrieve` | Retrieve |
| PUT | `/v1/software-release-procedure/{crId}/update` | Update |
| PUT | `/v1/software-release-procedure/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/software-release-procedure/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-software-deployment:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-software-deployment ./helm -n bian-business-support
```

Exposed through the platform Gateway at path prefix `/sd-software-deployment` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
